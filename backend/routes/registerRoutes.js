...
router.post("/register", async (req, res) => {
 ...
 try {
   ...
   // Insertar en MySQL y obtener el resultado (que contiene la ID)
   ...
   // Lógica de creación en MongoDB usando la ID real de MySQL
   await Usuario.create({
     playerId: newPlayerId, // <-- Usamos la ID real obtenida de MySQL
     username: username,
     aciertos: 0,
     errores: 0,
     palabrasFrecuentes: [],
...
// Esta es la ruta que será /api/user/user
router.post("/user", async (req, res) => {
 ...
 try {
   ...
   console.error("Error en /api/user/user:", error);
   res.status(500).json({ message: "Error interno del servidor." });
 }
});
